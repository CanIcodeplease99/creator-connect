...
const PostCardPublic = ({ post, onPreview }: PostCardPublicProps) => {
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 pt-5 pb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {post.creator.verified && <BadgeCheck size={15} className="text-primary" />}
          </div>
      ...
      {/* Caption - always visible as preview */}
      ...
      {post.media && (
        <div className="relative">
          <img src={post.media} alt="" className={`w-full aspect-[4/3] object-cover ${post.isLocked ? "blur-locked" : ""}`} />
          {post.isLocked && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-foreground/20">
              <Lock size={32} className="text-primary-foreground mb-2" />
...
const FeaturedPosts = () => {
  ...
  return (
    <section className="py-20 px-4 sm:px-6 max-w-2xl mx-auto">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
